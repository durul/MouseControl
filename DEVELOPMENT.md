...
- **System Integration:** Sets up the OS system tray icon, checks for macOS accessibility permissions, and binds system-wide dark/light mode states.
...
1. **Configuration Flow:** Command-line args are parsed (`_parse_cli_args`) to configure hardware specifics like `--hid-backend`.
